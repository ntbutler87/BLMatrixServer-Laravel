<?php

use App\Http\Controllers\MacroController;
use App\Http\Controllers\MatrixController;
use App\Http\Controllers\SceneController;
use App\Http\Controllers\ProfileController;
use Illuminate\Foundation\Application;
use Illuminate\Support\Facades\Route;
use Inertia\Inertia;

// Route::get('/matrix', function () {
Route::get('/', function () {
    return Inertia::render('Matrix', [
    ]);
});

Route::get('/matrix/status', [MatrixController::class, 'get_status'])->name('matrix.status');
Route::post('/matrix/video', [MatrixController::class, 'set_video'])->name('matrix.video.set');


Route::get('/dashboard', function () {
    return Inertia::render('Dashboard');
})->middleware(['auth', 'verified'])->name('dashboard');

Route::middleware('auth')->group(function () {
    Route::get('/profile', [ProfileController::class, 'edit'])->name('profile.edit');
    Route::patch('/profile', [ProfileController::class, 'update'])->name('profile.update');
    Route::delete('/profile', [ProfileController::class, 'destroy'])->name('profile.destroy');

    Route::resource('matrix', MatrixController::class);
    Route::resource('macro', MacroController::class);

    Route::resource('scene', SceneController::class)
        ->only(['index', 'edit', 'update']);

});

require __DIR__.'/auth.php';
